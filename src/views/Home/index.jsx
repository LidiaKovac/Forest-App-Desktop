...
const Home = () => {
  ...
  window.onblur = title_out
  return (
    <div className="home__wrap">
    <Confirmation show={show} giveUp={giveUp} setShow={setShow}/>
      <div className="home__content">
        {status === "normal" ? <i> Get back to work!</i> : status === 'gave up' && <i>You tree died.</i>}
        <div className="timer__wrap">
          <div className="timer__visual">
            {status === "normal" ? <img src={Tree} alt="tree" /> : status === 'gave up' && <img src={DeadTree} alt='tree'/>}
          </div>
          ...
          {!focusing && <button disabled={(minutes === 0 && hours === 0) && true} className="start__button" onClick={() => {
          ...
          {focusing && <div className="give-up__button" onClick={() => setShow(true)}>
            Give up
          </div>}
        </div>
      </div>
